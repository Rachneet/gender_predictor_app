...
def train(batch_size, num_epochs, learning_rate):
    path = "/home/rachneet/datasets/amazon/"
    ...
    training_set = MyDataset(path+"User_level_train.csv")
    # test_set = MyDataset(datapath + "test.csv")
    validation_set = MyDataset(path+"User_level_validation.csv")
    ...
    best_accuracy = 0
    output_file = open(path+"charCNN/train_logs.txt", "w")
    ...
    for epoch in range(num_epochs):
        ...
        if test_metrics["accuracy"] > best_accuracy:
            ...
            torch.save(model.state_dict(), path+ "charCNN/trained_model.pt")
